    ...
    for (let i = 0; i < queueData.length; i++) {
        ...
        if (isAuthenticated && userRoles.includes("ROLE_ADMIN")) {
                cells.push(<td key={"actions" + i} id={"action" + i}>{
                    <>
                        <ConfirmButton style={{width: "35%"}} onClick={() => confirmDoneEntity(queueData[i].id)}>Ferdig</ConfirmButton>
                        <DeleteButton style={{width: "35%"}} onClick={() => deleteQueueEntity(queueData[i].id)}>Slett</DeleteButton>
                    </>
                }</td>);
        }
